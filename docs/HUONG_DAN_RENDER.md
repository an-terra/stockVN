...
| `failed to solve: frontend grpc server closed unexpectedly` | Thường là **BuildKit** (Docker) mất kết nối tạm thời hoặc hết tài nguyên — **Deploy lại** trên Render; nếu lặp lại: **Clear build cache** (Settings → Build & Deploy) rồi deploy. Trên máy: tăng RAM Docker Desktop, **Restart Docker**. Trong repo, `Dockerfile` **không** dùng `# syntax=docker/dockerfile:1` để tránh bước kéo image parser riêng (giảm nguy cơ lỗi ở bước đó). |
